...
class ConversationEngine:
    ...
    def check_user_registration_status(self, telegram_id: str) -> tuple[bool, dict]:
        """Check if user is already registered"""
        try:
            import requests
            response = requests.get(f"{self.base_url}/api/check-registration/{telegram_id}")
            if response.status_code == 200:
                data = response.json()
                return data.get('is_registered', False), data.get('registration_data', {})
            return False, {}
        except Exception as e:
    ...
    async def generate_registration_link(self, telegram_id: str, telegram_username: str = "") -> str:
        ...
        try:
            ...
            is_registered, registration_data = self.check_user_registration_status(telegram_id)
            if is_registered:
                print(f"🔒 User {telegram_id} is already registered, skipping link generation")
    ...
    async def process_message(self, message: str, telegram_id: str, engagement_score: int, telegram_username: str = "") -> str:
        ...
        if intent == 'registration':
            ...
            if registration_url == "already_registered":
                if language == 'ms':
                    return (
                        "✅ Awak dah register untuk VIP Channel EzyAssist!\n\n"
                        "Team kami dah ada rekod pendaftaran awak. Jangan risau ye! 😊\n\n"
                        "Kalau ada masalah atau nak tanya status pendaftaran, boleh hubungi admin kami atau terus tanya kat sini.\n\n"
                        "Sementara tu, saya boleh bantu awak dengan:\n"
                        "• Tips trading dan strategy\n"
                        "• Info pasal broker terbaik\n"
                        "• Market analysis harian\n"
                        "• Forex education untuk Malaysian traders!"
                    )
                else:
                    return (
                        "✅ You're already registered for EzyAssist VIP Channel!\n\n"
                        "Our team has your registration on record. Don't worry! 😊\n\n"
                        "If you have any issues or want to check your registration status, feel free to contact our admin or ask me here.\n\n"
                        "In the meantime, I can help you with:\n"
                        "• Trading tips and strategies\n"
                        "• Best broker information\n"
                        "• Daily market analysis\n"
                        "• Forex education for Malaysian traders!"
                    )
