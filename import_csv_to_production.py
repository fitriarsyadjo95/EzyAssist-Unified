#!/usr/bin/env python3
"""
Script to import CSV data to the production admin panel on Railway
"""
import requests
import sys
import os
...
def import_csv_to_production(csv_file_path, base_url="https://ezyassist-unified-production.up.railway.app", username="admin@ezymeta.global", password=None):
    """
    Import CSV file to production admin panel on Railway
    """
    ...
    print(f"🔐 Logging in to PRODUCTION as {username}...")
    ...
    if 'admin_session' not in session.cookies:
        print("⚠️  No admin_session cookie received. Will try to proceed anyway...")
    ...
    print(f"📤 Uploading {csv_file_path} to PRODUCTION...")
...
if __name__ == "__main__":
    ...
    print("🚀 Starting CSV Import to PRODUCTION Admin Panel (Railway)")
    print("=" * 60)
    
    # Import the file
    success = import_csv_to_production(csv_file, password=password)
    
    if success:
        print("\n🎉 Import to PRODUCTION completed successfully!")
        print("You can now view the imported data in the production admin panel:")
        print("https://ezyassist-unified-production.up.railway.app/admin/registrations")
    else:
        print("\n❌ Import to PRODUCTION failed.")
