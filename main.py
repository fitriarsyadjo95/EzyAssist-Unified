...
def check_user_registration_status(telegram_id: str) -> tuple[bool, Optional[dict]]:
    """
    Check if user is already registered and return status
    Returns (is_registered, registration_data)
    """
    if not SessionLocal:
        return False, None
    ...
    try:
        ...
        # User has completed full registration (step 2)
        ...
        logger.error(f"Error checking registration status for {telegram_id}: {e}")
        return False, None
    finally:
        db.close()
...
@app.get("/api/check-registration/{telegram_id}")
async def api_check_registration_status(telegram_id: str):
    """API endpoint to check if user is already registered"""
    try:
        is_registered, registration_data = check_user_registration_status(telegram_id)
        return {
            "is_registered": is_registered,
            "registration_data": registration_data
        }
    except Exception as e:
        logger.error(f"Error in check registration API: {e}")
        return {"is_registered": False, "registration_data": {}}
